import React, {Component} from 'react';
import {Link} from 'react-router';

export default class Council extends Component {
  render() {
    const {params} = this.props;
    return (
      <div>
        <div className="breadcrumbs margin-bottom-50">
          <div className="container">
            <h1 className="pull-left">議會訊息</h1>
            <ul className="pull-right breadcrumb">
              <li><Link to={`/frontend/${params.name}`}>首頁</Link></li>
              <li>問政總覽</li>
            </ul>
          </div>
        </div>
        <div className="container s-results margin-bottom-50">
          <div className="row">
            <div className="col-md-2 hidden-xs related-search">
              <div className="row">
                <div className="col-md-12 col-sm-4">
                  <h3>分布年代</h3>
                  <ul className="list-unstyled">
                    <li><a href="#">2017 (21)</a></li>
                    <li><a href="#">2016 (18)</a></li>
                    <li><a href="#">2015 (4)</a></li>
                    <li><a href="#">2014 (21)</a></li>
                    <li><a href="#">2013 (18)</a></li>
                    <li><a href="#">2012 (4)</a></li>
                  </ul>
                  <hr/>
                </div>

              </div>
            </div>

            <div className="col-md-10">

              <div className="inner-results">
                <h3><a href="detail.php">提請修正【台北市都市更新自治條例】第15條第3項規定，俾被劃入山坡地，卻多數鄰接大馬路之住三老舊建物得依法申請都更，以維居住安全並利市容景觀兼顧弱勢等因，提請
                  公決。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php"> 建請市政府自今年（106年度）起，另闢財源，發放重陽敬老禮金；並自107年度重新編列法定預算，恢復發放重陽節敬老禮金致贈活動。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php">建請臺北市教育局針對國小全勤學生比照第一類市長獎頒發全勤市長獎予以肯定。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a
                  href="detail.php">建請臺北市政府擴大辦理汰購及新購電動機車補助方案，以落實淘汰老舊二行程機車、促進市民使用電動機車之意願，並順應巴黎氣候協定節能減碳對抗全球暖化與氣候變遷。</a>
                </h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php">提請修正【台北市都市更新自治條例】第15條第3項規定，俾被劃入山坡地，卻多數鄰接大馬路之住三老舊建物得依法申請都更，以維居住安全並利市容景觀兼顧弱勢等因，提請
                  公決。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php">提請修正【台北市都市更新自治條例】第15條第3項規定，俾被劃入山坡地，卻多數鄰接大馬路之住三老舊建物得依法申請都更，以維居住安全並利市容景觀兼顧弱勢等因，提請
                  公決。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php">提請修正【台北市都市更新自治條例】第15條第3項規定，俾被劃入山坡地，卻多數鄰接大馬路之住三老舊建物得依法申請都更，以維居住安全並利市容景觀兼顧弱勢等因，提請
                  公決。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php">提請修正【台北市都市更新自治條例】第15條第3項規定，俾被劃入山坡地，卻多數鄰接大馬路之住三老舊建物得依法申請都更，以維居住安全並利市容景觀兼顧弱勢等因，提請
                  公決。</a></h3>
                <p>民國106年6月20日</p>
              </div>


              <hr/>
              <div className="inner-results">
                <h3><a href="detail.php">提請修正【台北市都市更新自治條例】第15條第3項規定，俾被劃入山坡地，卻多數鄰接大馬路之住三老舊建物得依法申請都更，以維居住安全並利市容景觀兼顧弱勢等因，提請
                  公決。</a></h3>
                <p>民國106年6月20日</p>
              </div>
              <hr/>

              <div className="margin-bottom-30"></div>

              <div className="text-left">
                <ul className="pagination">
                  <li><a href="#">«</a></li>
                  <li className="active"><a href="#">1</a></li>
                  <li><a href="#">2</a></li>
                  <li><a href="#">3</a></li>
                  <li><a href="#">...</a></li>
                  <li><a href="#">157</a></li>
                  <li><a href="#">158</a></li>
                  <li><a href="#">»</a></li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}
